...
- [ ] **FUSE: panic "layer used after Close()" during unmount** (race condition): `Backend.Unmount()` calls `ext4.Unmount` (which triggers a kernel umount that flushes dirty pages through the FUSE block device), then `losetup --detach`, then `vol.Close()` which sets `layer.closed=true`. But the kernel's dirty page writeback generates FUSE write requests that are queued in go-fuse's goroutine pool. These in-flight writes can arrive at `layer.Write()` *after* `layer.Close()` has set `closed=true`, causing `checkWritable()` to panic at `layer.go:74`. Seen in `TestE2E_DataPersistsAcrossMountCycles` on the FUSE backend (not NBD). Stack trace: `fuseblockdev.(*deviceNode).Write` → `volume.Write` → `layer.Write` → `layer.checkWritable` → panic. The fix likely involves either: (a) waiting for all in-flight FUSE operations to drain before calling `vol.Close()` (go-fuse's `Server.Wait()` only waits for full unmount, not per-volume drain), or (b) making `layer.Write` return an error instead of panicking when `closed=true`, or (c) having the FUSE deviceNode check `vol.closed` and return `EIO` before forwarding writes. The Volume refcount system (`AcquireRef`/`ReleaseRef`) was designed for this, but the FUSE `Write` path doesn't acquire a ref per-request — it relies on the inode-level ref from `deviceNode`, which is released in `OnForget` (after the kernel drops the inode from cache), which may happen long after `vol.Close()`.
