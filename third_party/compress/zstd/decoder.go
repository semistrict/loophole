...
type Decoder struct {
	...
	// When concurrent == 1, singleBlock+singleMu bypass the channel to
	// avoid synctest "receive on channel from outside bubble" panics.
	singleBlock *blockDec
	singleMu    sync.Mutex
...
func NewReader(r io.Reader, opts ...DOption) (*Decoder, error) {
	...
	// Create decoders
	if d.o.concurrent == 1 {
		// Single-decoder fast path: no channel, just a mutex-guarded block.
		// Avoids Go synctest "receive on channel from outside bubble" panics
		// when DecodeAll is called from a different goroutine context than
		// the one that created the Decoder.
		dec := newBlockDec(d.o.lowMem)
		dec.localFrame = newFrameDec(d.o)
		d.singleBlock = dec
		d.decoders = make(chan *blockDec, 1)
		d.decoders <- dec // keep channel non-nil for Close() and nil checks
	} else {
...
func (d *Decoder) DecodeAll(input, dst []byte) ([]byte, error) {
	...
	var block *blockDec
	if d.singleBlock != nil {
		// Single-decoder path: use mutex instead of channel to avoid
