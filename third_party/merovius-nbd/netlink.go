...
func Loopback(ctx context.Context, d Device, size uint64, opts ...LoopbackOption) (idx uint32, wait func() error, err error) {
	...
	wait = func() error {
		...
		if e := nbdnl.Disconnect(idx); e != nil && err == nil && !isAlreadyDisconnectedError(e) {
...
func isAlreadyDisconnectedError(err error) bool {
	return errors.Is(err, unix.EINVAL) || errors.Is(err, unix.ENODEV) || errors.Is(err, unix.ENOENT)
}
