...
func serverHandshakeDynamic(ctx context.Context, rw io.ReadWriter, provider ExportProvider) (connParameters, error) {
	parms := connParameters{
		BlockSizes: defaultBlockSizes,
	}
	return parms, do(rw, func(e *encoder) {
		...
		for {
			...
			switch o := o.(type) {
			case *optExportName:
				exp, findErr := provider.FindExport(ctx, o.name)
				if findErr != nil {
					encodeReply(e, code, &repError{errUnknown, ""})
					continue
				}
			...
			case *optAbort:
				...
				e.check(errors.New("client aborted negotiation"))
			case *optList:
				exports, listErr := provider.ListExports(ctx)
				if listErr != nil {
					encodeReply(e, code, &repError{errUnknown, ""})
					continue
				}
