...
pub trait S3Access: Send + Sync + Clone + 'static {
    ...
    /// Put bytes only if the key does not already exist (S3 conditional write).
    /// Returns Ok(true) if created, Ok(false) if the key already existed.
    fn put_bytes_if_not_exists<'a>(
        &'a self,
        bucket: &'a str,
        key: &'a str,
        body: Vec<u8>,
...
impl S3Access for Client {
    ...
    async fn put_bytes_if_not_exists(
        ...
        let _timing = crate::metrics::timing!("s3.put_conditional");
        let result = self
            .put_object()
            .bucket(bucket)
            .key(key)
        ...
        match result {
            Ok(_) => Ok(true),
            Err(err) => {
                // S3 returns 412 Precondition Failed when object already exists.
                ...
                Err(err).with_context(|| format!("conditional put_object failed for {key}"))
            }
        }
    }
}
...
pub struct State {
    ...
    pub block_size: u64,
    pub volume_size: u64,
    /// ISO-8601 timestamp; present means this store is frozen (no further writes).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub frozen_at: Option<String>,
    /// Child store IDs (populated by VolumeManager after freeze).
