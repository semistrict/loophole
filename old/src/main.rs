...
use loophole::fs;
use loophole::volume_manager::VolumeManager;
use loophole::{assert, cache, metrics, names, store};
use std::sync::Arc;
...
enum Command {
    ...
    /// Create a new volume and format a kernel ext4 filesystem on it (FUSE + loop + ext4)
    Format {
        /// Volume name
        #[arg(long)]
        name: String,
    ...
    /// Mount a volume as a kernel ext4 filesystem (FUSE + loop + ext4)
    Mount {
        /// Volume name
        #[arg(long)]
        name: String,
    ...
    /// Snapshot the volume via the FUSE mount
    Snapshot {
        /// Mount point of the running filesystem
        mountpoint: PathBuf,

        /// Volume name for the snapshot
        #[arg(long)]
        name: String,
    },

    /// Clone the volume via the FUSE mount
    Clone {
        /// Mount point of the running filesystem
        mountpoint: PathBuf,

        /// Volume name for the clone
        #[arg(long)]
        name: String,
    },
}
...
/// Find the volume name by listing the FUSE mount root (the only non-.loophole entry).
fn find_volume_name(fuse_mount: &std::path::Path) -> anyhow::Result<String> {
    for entry in std::fs::read_dir(fuse_mount).context("reading FUSE mount root")? {
        let entry = entry?;
    ...
    anyhow::bail!("no volume found in FUSE mount at {}", fuse_mount.display())
}

/// Trigger a snapshot by creating `.loophole/snapshots/<volume_name>/<new_name>` and reading back status.
fn trigger_snapshot(mount: &std::path::Path, volume_name: &str, new_name: &str) -> anyhow::Result<()> {
    let ctl_path = mount.join(".loophole/snapshots").join(volume_name).join(new_name);
...
/// Trigger a clone by creating `.loophole/clones/<volume_name>/<new_name>` and reading back status.
fn trigger_clone(mount: &std::path::Path, volume_name: &str, new_name: &str) -> anyhow::Result<()> {
    let ctl_path = mount.join(".loophole/clones").join(volume_name).join(new_name);
    std::fs::write(&ctl_path, b"").with_context(|| format!("creating {}", ctl_path.display()))?;
    let status = std::fs::read_to_string(&ctl_path)
        .with_context(|| format!("reading {}", ctl_path.display()))?;
    ...
    match cli.command {
        ...
        Command::Store { command } => match command {
            ...
            StoreCommand::Snapshot {
                ...
                let volume_name = find_volume_name(&mountpoint)?;
                trigger_snapshot(&mountpoint, &volume_name, &new_store)?;
            }

            StoreCommand::Clone { mountpoint, clone } => {
                syncfs_mount(&mountpoint)?;
                let volume_name = find_volume_name(&mountpoint)?;
                trigger_clone(&mountpoint, &volume_name, &clone)?;
            }
            ...
            StoreCommand::Mount {
                ...
                #[cfg(feature = "block-fuse")]
                {
                    ...
                    vm.add_volume(store_id.clone(), store);
                    let filesystem = fs::Loophole::new(vm);
        ...
        // =============================================================
        // High-level ext4 commands (FUSE + loop + kernel ext4)
        // =============================================================
        Command::Format {
            name: volume_name,
            block_size,
            ...
            // 2. Write volume name → store UUID mapping (conditional: fails if name exists).
            names::create_volume_ref(&client, &cli.bucket, &cli.prefix, &volume_name, &store_id)
                .await?;

            info!(volume = %volume_name, store = %store_id, "store created, formatting ext4 on volume");

            // 3. Mount FUSE in a temp directory, create loopback, mkfs.ext4.
            ...
            vm.add_volume(volume_name.clone(), store);
            let filesystem = fs::Loophole::new(vm);
            ...
            let volume_path = fuse_path.join(&volume_name);
            anyhow::ensure!(volume_path.exists(), "FUSE volume file not found at {}", volume_path.display());
            ...
            info!(volume = %volume_name, store = %store_id, "format complete — volume has empty ext4 filesystem");
        }
        ...
        Command::Mount {
            ...
            // Resolve volume name → store UUID.
            let vol_ref = names::read_volume_ref(&client, &cli.bucket, &cli.prefix, &volume_name)
                .await
                .with_context(|| format!("resolving volume name {volume_name:?}"))?;
            let store_id = vol_ref.uuid;
            info!(volume = %volume_name, store = %store_id, "resolved volume name");
            ...
            //    because ext4 would shadow it and make .loophole inaccessible).
            let fuse_dir = tempfile::tempdir().context("creating temp FUSE mountpoint")?;
            let fuse_path = fuse_dir.path().to_path_buf();
        ...
        Command::Snapshot {
            ...
            let volume_name = find_volume_name(&fuse_mount)?;
            info!(fuse_mount = %fuse_mount.display(), volume = %volume_name, "found FUSE mount");
            trigger_snapshot(&fuse_mount, &volume_name, &snapshot_name)?;
        }
        ...
        Command::Clone {
            ...
            let volume_name = find_volume_name(&fuse_mount)?;
            info!(fuse_mount = %fuse_mount.display(), volume = %volume_name, "found FUSE mount");
            trigger_clone(&fuse_mount, &volume_name, &clone_name)?;
        }
    }
