...
func TestE2E_Sandboxd_AttachExistingVolume(t *testing.T) {
	skipE2E(t)
	_, _ = setupBusyboxVolume(t, "sandboxd-attach")
	...
	require.NoError(t, json.Unmarshal(data, &sb))
	require.Equal(t, "running", sb.State)
	require.Equal(t, "adopted", sb.OwnerMode)
	...
	client.jsonRequest(t, http.MethodDelete, fmt.Sprintf("/v1/sandboxes/%s", sb.ID), nil)

	// Attached rootfs should remain owned by the original loophole process.
	r := sandboxExec(t, "sandboxd-attach", "cat+/marker.txt")
	require.Equal(t, 0, r.ExitCode)
	require.Equal(t, "chrooted\n", r.Stdout)
}

func TestE2E_Sandboxd_AttachExistingVolumeChurn(t *testing.T) {
	skipE2E(t)
	_, _ = setupBusyboxVolume(t, "sandboxd-attach-churn")

	socket := startSandboxd(t)
	client := newSandboxdClient(t, socket)
	startLogSize := int64(0)
	if info, err := os.Stat(sandboxdLogPath()); err == nil {
		startLogSize = info.Size()
	}

	for i := 0; i < 10; i++ {
		data := client.jsonRequest(t, http.MethodPost, "/v1/sandboxes", map[string]any{
			"name": fmt.Sprintf("attach-churn-%d", i),
			"source": map[string]any{
				"kind":   "volume",
				"volume": "sandboxd-attach-churn",
				"mode":   "attach",
			},
		})
		var sb sandboxRecord
		...
		r := sandboxExec(t, "sandboxd-attach-churn", "cat+/marker.txt")
		require.Equal(t, 0, r.ExitCode)
		require.Equal(t, "chrooted\n", r.Stdout)
	}

	logData, err := os.ReadFile(sandboxdLogPath())
	require.NoError(t, err)
	logTail := string(logData)
	if startLogSize > 0 && startLogSize < int64(len(logData)) {
		logTail = string(logData[startLogSize:])
	}
	for _, needle := range []string{
		"runsc delete:",
		"gofer is still running",
		"sandbox is still running",
		"runsc kill failed",
	} {
		require.False(t, strings.Contains(logTail, needle), "sandboxd log contains teardown regression marker %q:\n%s", needle, logTail)
	}
}
