...
func newS3TestStore(t *testing.T) *Store {
	t.Helper()
	skipWithoutS3(t)
	...
	drv, err := NewS3Driver(t.Context(), env.ResolvedStore{
		StoreURL: os.Getenv("S3_ENDPOINT") + "/" + bucket,
		Bucket:   bucket,
		Endpoint: os.Getenv("S3_ENDPOINT"),
	})
	require.NoError(t, err)
	return New(drv).At("test-" + uuid.NewString())
}
...
func TestGetWithRetryRetriesTransientReadErrors(t *testing.T) {
	store := &Store{
		readRetries:      2,
...
func TestGetWithRetryStopsOnCanceledContext(t *testing.T) {
