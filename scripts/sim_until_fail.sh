...
# Repeatedly run TestSimulation until a failure appears.
# Uses make test (never direct go test) so lwext4/CGO flags are consistent.

RUN_TEST="${RUN_TEST:-TestSimulation}"
SIM_OPS_FIXED="${SIM_OPS_FIXED:-300}"
SIM_PAGES_FIXED="${SIM_PAGES_FIXED:-256}"

LOG_DIR="${SIM_FUZZ_LOG_DIR:-/tmp/sim-fuzz}"
      ...
      if [[ ! -f "$FAIL_FLAG" ]]; then
        {
          ...
          echo
          echo "Repro:"
          echo "SIM_SEED=$seed SIM_OPS=$SIM_OPS_FIXED SIM_PAGES=$SIM_PAGES_FIXED SIM_NODES=$nodes SIM_CRASH_RATE=$crash SIM_FAULT_RATE=$fault SIM_TIMELINES=$timelines make test RUN=$RUN_TEST"
        } > "$FAIL_SUMMARY"
        touch "$FAIL_FLAG"
      fi
      return 1
